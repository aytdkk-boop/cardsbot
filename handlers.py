...
from keyboards import close_inline_kb, main_menu_kb
from utils import (
    forward_geocode,
    reverse_geocode,
    parse_coordinates,
...
async def handle_query(message: types.Message, state: FSMContext):
    ...
    # Пробуем как координаты, иначе — как адрес
    coords = parse_coordinates(text)
    if coords:
        info = await reverse_geocode(coords[0], coords[1])
    else:
        info = await forward_geocode(text)
...
async def global_error_handler(event: ErrorEvent):
    """
    Ловит ЛЮБУЮ ошибку в любом хэндлере.
    - пишет traceback в консоль/лог
    - отправляет текст ошибки в чат пользователю
    - опционально уведомляет администратора (ADMIN_ID)
    """
    exc = event.exception
    ...
    # 3. Отправляем текст ошибки пользователю
    if chat_id:
    ...
    return True  # помечаем ошибку обработанной